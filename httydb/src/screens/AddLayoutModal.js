    ...
    render() {
        ...
        if (layouts){
            layoutTemplateList = layouts.map((layout, index) => {
                return(<Card key={index}><Text>{layout}</Text></Card>)
            })
        }

        if (recent){
            recentLayoutList = recent.map((rec, index) => {
                return(<Card key={index}><Text>{rec}</Text></Card>)
            })
        }
        ...
        return (
            <View>
                ...
                <TextInput placeholder="eg. MyLayout" style={styles.TextBoxStyle}></TextInput>
                <Text style={styles.TextStyle}>Number Of Rows:</Text>
                <TextInput placeholder="eg. for 20 seats, enter 10 rows" style={styles.TextBoxStyle}></TextInput>
                <TouchableOpacity>
