...
class db_impl implements Database {
  ...
  public updateTeammamber(team_member: Team_member): Promise<number> {
    return this.getDB()
      .then(db =>
        db.executeSql(
          'UPDATE team_member SET name = ?, email = ?, phone = ?, gender = ?, weight = ?, height = ? , side_preference = ?, active = ?, emergency_cont = ?' +
  ...
  public removeTeammamber(team_member: Team_member): Promise<number> {
    return this.getDB()
      .then(db =>
        db.executeSql(
          'UPDATE team_member SET active = Inactive' +
            '  WHERE team_member_id = ?;',
          [team_member.id],
        ),
      )
      ...
      .then(([results]) => {
        if (results !== undefined) {
          ...
          for (let i = 0; i < results.rows.length; i++) {
            ...
            paddlers[side][position] = new Team_member(
              name,
              email,
              phone,
              gender,
  ...
  public getBoatLayoutByID(id): Promise<Boat_Layout> {
    return this.getDB()
      ...
      .then(layout => {
        return this.getPaddlersOnBoat(layout.id, layout.num_paddlers).then(
          paddlersonboat => {
  ...
  public getallBoatLayouts(): Promise<Boat_Layout[]> {
    return this.getDB()
      .then(db => db.executeSql('SELECT * FROM Boat_Layout ORDER BY date ;'))
      .then(([results]) => {
        if (results !== undefined) {
    ...
    return this.getDB()
      ...
      .then(([results]) => {
        if (results !== undefined) {
          ...
          for (let i = 0; i < results.rows.length; i++) {
            const data = results.rows.item(i);
            let {timestamp, lat, long} = data;
            map_points.push(new Map_Point(timestamp, lat, long));
          }
          return map_points;
        } else return Promise.reject();
      });
    ...
    return this.getDB().then(db =>
      db
        .executeSql(
          ...
          [race.layout.id, race.date, race.name, race.duration, race.duration],
        )
        .then(([results]) => {
  ...
  public getRaceById(id): Promise<Race> {
    return this.getDB()
      .then(db => db.executeSql('SELECT * FROM race WHERE r.race_id = ?'))
      .then(([results]) => {
        if (results != undefined) {
          const data = results.rows.item(0);
          let {layout_id, date, name, duration, distance, race_id} = data;
          return new Race(date, name, distance, duration, race_id);
        } else return Promise.reject();
      })
      .then(race => {
  ...
  public getAllRaces(): Promise<Race[]> {
    return this.getDB()
      .then(db => db.executeSql('SELECT * FROM race ORDER BY race_date'))
      .then(([results]) => {
        if (results !== undefined) {
  ...
  public getRecentRaces(num = 3): Promise<Race[]> {
    return this.getDB()
      .then(db =>
        db.executeSql('SELECT * FROM race ORDER BY race_date LIMIT ?', [num]),
      )
      .then(([results]) => {
        if (results !== undefined) {
          let races: [Promise<Race>];
  ...
  public getRace(id): Promise<Race> {
    return this.getDB()
      ...
      .then(([results]) => {
        if (results != undefined) {
          const data = results.rows.item(0);
          let {layout_id, name, date, duration, distance, race_id} = data;
          return new Race(date, name, distance, duration, race_id);
        } else return Promise.reject();
      })
      .then(race => {
